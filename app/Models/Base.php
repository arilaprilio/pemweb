...
class Base extends Model
{
    ...
    public function cekOrdersUsersProducts()
    {
        return $this->db->table("orders")
                    ->select('orders.*, users.nama AS nama_user, users.email AS user_email')
                    ->join('users', 'users.id = orders.user_id')
                    ->orderBy('orders.id', 'DESC')
                    ->get()->getResultArray();
    }

    public function cekOrdersItemsProducts()
    {
        return $this->db->table("order_items")
                    ->select('order_items.*, product.nama AS nama_produk, product.harga AS harga_produk, product.foto AS foto_produk')
