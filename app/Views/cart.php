            ...
            <div class="offcanvas__bottom">
               <div class="offcanvas__footer d-flex align-items-center justify-content-between">
                  <div class="offcanvas__currency-wrapper currency">
                     <span class="offcanvas__currency-selected-currency tp-currency-toggle" id="tp-offcanvas-currency-toggle">Currency : IDR</span>
                     ...
                     <div class="cartmini__widget-item">
                         <div class="cartmini__thumb">
                           <a href="product-details.html">
                              <img src="<?=site_url();echo $cart['foto_produk']?>" alt="" width="250px" height="250px">
                           </a>
                         </div>
                         <div class="cartmini__content">
                           <h5 class="cartmini__title"><a href="product-details.html"><?=esc($cart['nama_produk'])?></a></h5>
                           <div class="cartmini__price-wrapper">
                              <span class="cartmini__price">Rp <?=number_format(esc($cart['harga_produk']), 0, ',', '.')?></span>
                              <span class="cartmini__quantity">x<?=esc($cart['jumlah'])?></span>
                           </div>
                         </div>
                         <a href="<?=site_url()?>cart/delete?id=<?=esc($cart['id'])?>" class="cartmini__del"><i class="fa-regular fa-xmark"></i></a>
                     </div>
                     <?php
                     }
                     ?>
     ...
     <!-- cart mini area end -->
                     ...
                     <div class="tp-cart-list mb-25 mr-30">
                        <table class="table">
                           ...
                           <tbody>
                              ...
                              foreach ($carts as $cart) {
                              ?>
                              <tr data-cart-id="<?= esc($cart['id']) ?>">
                                 <!-- img -->
                                 <td class="tp-cart-img"><a href="<?=site_url()?>product/<?=esc($cart['produk_id'])?>"> <img src="<?=site_url();echo $cart['foto_produk']?>" alt=""></a></td>
                                 <!-- title -->
                                 <td class="tp-cart-title"><a href="<?=site_url()?>product/<?=esc($cart['produk_id'])?>"><?=esc($cart['nama_produk'])?></a></td>
                                 <!-- price -->
                                 <td class="tp-cart-price"> <span data-price="<?= esc($cart['harga_produk']) ?>"> Rp <?= number_format(esc($cart['harga_produk']),0,',','.') ?> </span> </td>
                                 <!-- quantity -->
                                 <td class="tp-cart-quantity">
                                    <div class="tp-product-quantity mt-10 mb-10">
                                       ...
                                       <input type="text" class="tp-cart-input" name="qty[]" value="<?= $cart['jumlah'] ?>">
                                       <input type="hidden" name="product_id[]" value="<?= $cart['produk_id'] ?>">
                                 ...
                                 <td class="tp-cart-action">
                                    <a href="<?=site_url()?>cart/delete?id=<?=esc($cart['id'])?>" class="tp-cart-action-btn">
                                       <svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <div class="col-xl-3 col-lg-4 col-md-6">
                     <div class="tp-cart-checkout-wrapper">
                        <div class="tp-cart-checkout-total d-flex align-items-center justify-content-between">
                           <span>Total</span>
                              <span id="cartTotal">Rp <?=number_format(esc($total), 0, ',', '.')?></span>
                        </div>
                        <div class="tp-cart-checkout-proceed">
                           <a href="<?=site_url()?>checkout" class="tp-cart-checkout-btn w-100">Lanjutkan Checkout</a>
                        </div>
                     </div>
                  </div>
               </div>
      ...
      function recalcCartTotal(){
         ...
         document.querySelectorAll('tr[data-cart-id]').forEach(row => {
            ...
            row.querySelector('[data-price]').dataset.price, 10
            );
            const qty = parseInt(
            row.querySelector('.tp-cart-input').value, 10
            ) || 1;
            total += price * qty;
         });
      ...
      document.querySelectorAll('.tp-cart-plus, .tp-cart-minus')
         .forEach(btn => btn.addEventListener('click', async e => {
            const row = e.target.closest('tr[data-cart-id]');
            const input = row.querySelector('.tp-cart-input');
            let qty = parseInt(input.value,10) || 1;

            if (e.target.classList.contains('tp-cart-plus')) qty++;
            else if (qty>1) qty--;
            ...
            await fetch('<?= site_url("api/updateCart") ?>', {
            method: 'POST',
